...
const ChatContainner = () => {
 ...
 const {AuthUser,onlineUsers} = useContext(AuthContext)
  ...
  return selectedUser ? (
    <div className='h-full flex flex-col overflow-scroll backdrop-blur-lg'>
      {/*-------------- header---------------- */}
      ...
      {/*---------------- chat area------------------ */}
      <div className='flex-1 flex flex-col h-[calc(100% - 120px)] overflow-y-scroll p-3 pb-6'>
       {
        messages.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== AuthUser._id && 'flex-row-reverse'}`}>
           ...
           <div className='text-center text-xs'>
            ...
            <p className='text-gray-500'>{FomatMessageTime(msg.createdAt)}</p>
           </div>
          </div>
        ))
       }
